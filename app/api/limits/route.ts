...
export async function GET(req: NextRequest) {
  ...
  // Fetch live prices for the demo data
  try {
    const prices = await fetchLivePrices();
    const demo = buildData();
    // Update supportedTokens with live prices
    demo.supportedTokens = demo.supportedTokens.map(t => ({
      ...t, price: prices[t.symbol] ?? t.price,
    }));
    // Update currentPrice in open orders
    demo.openOrders = demo.openOrders.map(o => ({
      ...o,
      currentPrice: prices[o.outputToken] ?? o.currentPrice,
    }));
    return NextResponse.json({ ...demo, source: 'estimated' });
  } catch {
    return NextResponse.json({ ...buildData(), source: 'demo' });
  }
}
