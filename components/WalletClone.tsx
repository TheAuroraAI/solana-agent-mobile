      ...
      {cloneTarget && (
        <div className="mb-3 p-3 bg-violet-500/10 border border-violet-500/20 rounded-xl">
          ...
          <div className="flex gap-2">
            <a
              href={`/chat?${demo ? 'demo=true&' : ''}prompt=${encodeURIComponent(`Clone the portfolio of wallet ${cloneTarget.address} (${cloneTarget.label}). Their allocation: ${cloneTarget.solPct}% SOL${cloneTarget.tokens.slice(0, 3).map(t => `, ${t.pct}% ${t.symbol}`).join('')}. Generate specific swap actions to match this allocation.`)}`}
